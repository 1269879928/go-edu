...
func NewQiniuUpload(localFile, key, accessKey, secretKey, bucket string) Uploader {
	return &QiniuUpload{
		LocalFile: localFile,
		AccessKey: accessKey,
		SecretKey: secretKey,
...
func (ts *QiniuUpload)Upload() (savePath string, err error)  {
	localFile := ts.LocalFile
	bucket := ts.Bucket
	key := ts.Key
	...
	mac := qbox.NewMac(ts.AccessKey, ts.SecretKey)
